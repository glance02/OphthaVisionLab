...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., use_flash=False):
        ...
        self.use_flash = use_flash # whether to use flash attention
        if self.use_flash and (version.parse(torch.__version__) < version.parse('2.0.0')):
            ...
            print(f'will disable flash attention')
            self.use_flash = False

        if self.use_flash:
            self.flash_attn = FlashAttention()
            print(f"will use Flash Attention.")
...
class PatchSampler(object):
    ...
    def __call__(self, pmap):
        ...
        feat_idx += 1 # class embedding concat before image embedding
...
class VisionTransformer(nn.Module):
    """ Vision Transformer
    add param: use_flash: whether to use flash attention, which requires pytorch version greater than 2.0.0
    """
    ...
    def forward(self, x, return_all_tokens=None, mask=None, maps=None):
        ...
        if maps is not None and self.mask_ratio < 1:
            ...
            active_idx = self.patch_sampler(maps) # mask patches based on the given saliency map, return [B, 148]
            active_idx = active_idx.unsqueeze(-1).repeat(1, 1, self.embed_dim) # default in Sp16: return [B, 148, 384]
            x = torch.gather(x, dim=1, index=active_idx) # default in Sp16: [B, 196, 384] -> [B, 148, 384] # select subset by the given saliency map
    ...
    def get_intermediate_layers(self, x, n=1):
        x = self.prepare_tokens(x)
        # we return the output tokens from `n` last blocks
        output = []
        for i, blk in enumerate(self.blocks):
            x = blk(x)
