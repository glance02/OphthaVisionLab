...
| 属性 | 值 |
|------|-----|
| **模型文件** | `checkpoints/single_cls/checkpoint_teacher_linear.pth` |
...
| 属性 | 值 |
|------|-----|
| **模型文件** | `checkpoints/seg/checkpoint_108_linear.pth` |
| **预训练权重** | `pretrain_weights/VFM_Fundus_weights.pth` |
...
| 训练模型 | `checkpoints/single_cls/checkpoint_teacher_linear.pth` | 微调后的分类器权重 |
...
python tools/inference_binary.py \
    --checkpoint checkpoints/single_cls/checkpoint_teacher_linear.pth \
...
|------|------|------|
| 预训练权重 | `pretrain_weights/VFM_Fundus_weights.pth` | ViT 编码器基础权重 |
| 训练模型 | `checkpoints/seg/checkpoint_108_linear.pth` | 训练的分割头权重 |
